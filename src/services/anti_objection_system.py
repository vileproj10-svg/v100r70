...
class AntiObjectionSystem:
    ...
    def create_comprehensive_objection_handling(self, segmento: str, produto: str, web_research: Dict = None, social_analysis: Dict = None) -> Dict[str, Any]:
        """Cria sistema abrangente de tratamento de objeções"""
        return self.create_objection_handling_system(segmento, produto, web_research=web_research, social_analysis=social_analysis)

    def create_objection_handling_system(self, segmento: str, produto: str, web_research: Dict = None, social_analysis: Dict = None) -> Dict[str, Any]:
        """Cria sistema de tratamento de objeções"""
        # Placeholder for the actual implementation if it were different from the comprehensive one.
        # For now, it's assumed to be the same as the comprehensive system.
        return self.create_comprehensive_objection_handling(segmento, produto, web_research=web_research, social_analysis=social_analysis)


    def create_objection_handling_system_with_product(self, segmento, produto, context=None):
        """Cria sistema de tratamento de objeções com produto"""
        try:
            logger.info(f"🛡️ Criando sistema anti-objeção para {segmento} - {produto}")

            from services.ai_manager import ai_manager

            prompt = f"""
Crie um sistema completo anti-objeção para o segmento "{segmento}" com produto "{produto}".

Identifique e trate as 5 principais objeções:
1. TEMPO ("Não tenho tempo")
2. DINHEIRO ("É muito caro") 
3. CONFIANÇA ("Não confio")
4. NECESSIDADE ("Não preciso")
5. ESPECÍFICA (objeção única do segmento)

Para cada objeção:
- Raiz emocional real
- Contra-ataque específico
- Scripts de neutralização
- Histórias viscerais
- Drives mentais para usar
...
{{
    "sistema_anti_objecao": {{
        "objecoes": [
            {{
                "tipo": "TEMPO/DINHEIRO/CONFIANÇA/NECESSIDADE/ESPECÍFICA",
                "objecao_especifica": "Frase exata que o cliente fala",
                "raiz_emocional": "Medo real por trás",
                "contra_ataque": "Resposta direta",
                "scripts_personalizados": ["script1", "script2"],
                "drives_mentais": ["drive1", "drive2"],
                "historias_viscerais": ["história1", "história2"]
            }}
        ],
        "arsenal_emergencia": ["resposta1", "resposta2"],
        "sequencia_neutralizacao": "Ordem ideal de tratamento"
    }}
}}
"""
            ...
            if response:
                import json
                try:
                    anti_objection_data = json.loads(response)
                    return anti_objection_data.get('sistema_anti_objecao', {})
                except:
                    return self._create_fallback_anti_objection(segmento, produto)
            else:
                return self._create_fallback_anti_objection(segmento, produto)
    ...
    def _create_fallback_anti_objection(self, segmento, produto):
        """Cria sistema anti-objeção de fallback"""
        return {
            "objecoes": [
                {
                    "tipo": "TEMPO",
                    "objecao_especifica": "Não tenho tempo para implementar",
                    "raiz_emocional": "Medo de mais trabalho",
                    "contra_ataque": f"Entendo que tempo é precioso. Mas quanto tempo você perde SEM dominar {produto}?",
                    "scripts_personalizados": [
                        f"Você sabia que {produto} economiza tempo a longo prazo?",
                        f"Quanto tempo você perdeu tentando resolver isso sem {produto}?"
                    ],
                    "drives_mentais": ["Economia de tempo", "Eficiência"],
                    "historias_viscerais": [
                        f"História de quem economizou 5 horas por semana com {produto}",
                        f"Caso de quem perdeu oportunidade por não conhecer {produto}"
                    ]
                },
                {
                    "tipo": "DINHEIRO", 
                    "objecao_especifica": "Preciso pensar melhor sobre investimento",
                    "raiz_emocional": "Medo de perder dinheiro",
                    "contra_ataque": f"Quanto você perde por MÃO dominar {produto}? O custo de não agir é maior.",
                    "scripts_personalizados": [
                        f"O investimento em {produto} se paga em quanto tempo?",
                        f"Qual o custo de continuar sem {produto}?"
                    ],
                    "drives_mentais": ["ROI", "Custo de oportunidade"],
                    "historias_viscerais": [
                        f"Cliente que recuperou investimento em {produto} em 30 dias",
                        f"História de quem perdeu dinheiro por não ter {produto}"
                    ]
                }
            ],
            "arsenal_emergencia": [
                f"Se não for agora com {produto}, quando será?",
                f"O que mudou desde a última vez que você pensou nisso?"
            ],
            "sequencia_neutralizacao": "TEMPO -> DINHEIRO -> CONFIANÇA -> NECESSIDADE"
        }
