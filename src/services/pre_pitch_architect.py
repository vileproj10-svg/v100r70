...
"""
    ...
    def create_pre_pitch_strategy(self, segmento: str, produto: str, web_research: Dict = None, social_analysis: Dict = None) -> Dict[str, Any]:
        """Cria estratégia de pré-pitch"""
        return self.create_pre_pitch_system(segmento, produto)

    def create_pre_pitch_system(self, segmento: str, produto: str) -> Dict[str, Any]:
        """Cria sistema completo de pré-pitch"""
        try:
            logger.info(f"🎯 Criando pré-pitch para {segmento} - {produto}")

            from services.ai_manager import ai_manager

            prompt = f"""
Crie uma estratégia completa de pré-pitch para o segmento "{segmento}" com produto "{produto}".

O pré-pitch deve seguir esta sequência psicológica:
1. QUEBRA - Destruir ilusão confortável (3-5 min)
2. EXPOSIÇÃO - Revelar ferida real (3-5 min)  
3. INDIGNAÇÃO - Criar revolta produtiva (3-5 min)
4. VISLUMBRAR - Mostrar o possível (3-5 min)
5. TENSÃO - Amplificar o gap (3-5 min)
6. NECESSIDADE - Tornar mudança inevitável (3-5 min)

Para cada fase:
- Objetivo específico
- Drivers mentais a usar
- Narrativa/script
...
{{
    "pre_pitch_strategy": {{
        "sequencia_psicologica": [
            {{
                "fase": "quebra/exposicao/indignacao/vislumbrar/tensao/necessidade",
                "objetivo": "O que conseguir",
                "duracao": "3-5 minutos",
                "drivers_utilizados": ["driver1", "driver2"],
                "narrativa": "Script completo da fase",
                "resultado_esperado": "Estado emocional desejado",
                "transicao": "Como conectar à próxima fase"
            }}
        ],
        "roteiro_completo": {{
            "abertura": "Script de abertura",
            "desenvolvimento": "Corpo principal", 
            "fechamento": "Ponte para oferta"
        }},
        "timing_total": "15-30 minutos",
        "pontos_criticos": ["momento1", "momento2"]
    }}
}}
"""
            ...
            if response:
                ...
                try:
                    ...
                    return prepitch_data.get('pre_pitch_strategy', {})
                except:
            ...
            logger.error(f"❌ Erro ao criar pré-pitch: {e}")
            return self._create_fallback_prepitch(segmento, produto)

    def _create_fallback_prepitch(self, segmento, produto):
        """Cria pré-pitch de fallback"""
        return {
            "sequencia_psicologica": [
                {
                    "fase": "quebra",
                    ...
                    "duracao": "3-5 minutos",
                    "drivers_utilizados": ["Diagnóstico Brutal"],
                    "narrativa": f"Você acha que {segmento} é simples? Que é só uma questão de talento? Errado! A verdade é que {segmento} tem segredos que poucos conhecem.",
                    "resultado_esperado": "Desconforto produtivo",
                    "transicao": "Conectar com exposição da ferida real"
                },
                {
                    "fase": "exposicao",
                    "objetivo": "Revelar ferida real",
                    "duracao": "3-5 minutos",
                    "drivers_utilizados": ["Evidência Incontestável"],
                    "narrativa": f"Você já perdeu oportunidades por não dominar completamente {produto}? Já se sentiu frustrado por não conseguir os resultados que outras pessoas conseguem?",
                    "resultado_esperado": "Reconhecimento da necessidade",
                    "transicao": "Amplificar a frustração"
                }
            ],
            "roteiro_completo": {
                "abertura": f"Você está pronto para descobrir a verdade sobre {segmento}?",
                "desenvolvimento": f"A maioria das pessoas em {segmento} comete os mesmos erros...",
                "fechamento": f"Você está pronto para dominar {produto} de uma vez por todas?"
            },
            "timing_total": "15-20 minutos",
            "pontos_criticos": ["Exposição da ferida real", "Momento de maior tensão"]
        }
