...
class PsychologicalAgentsSystem:
    ...
    def execute_complete_psychological_analysis(
        ...
        for agent_name, agent in self.agents.items():
            # Use o método _execute_agent para gerenciar a execução e fallback
            agent_result = self._execute_agent(agent_name, data, session_id)
            results['agents_results'][agent_name] = agent_result

            # Salva resultado de cada agente, se não houve erro de recursão
            if not agent_result.get('fallback_used', False):
                salvar_etapa(f"agente_{agent_name}", agent_result, categoria="analise_completa")
            else:
                # Se houve recursão, o erro já foi logado, mas podemos salvar um registro aqui
                salvar_erro(f"agente_{agent_name}_recursion_error", agent_result.get('error', 'Unknown recursion error'), contexto=data)
    ...
    def _execute_agent(self, agent_name: str, data: Dict[str, Any], session_id: str = None) -> Dict[str, Any]:
        """Executa agente específico com dados e proteção contra recursão"""

        # CORREÇÃO CRÍTICA: Previne recursão infinita
        if agent_name not in self.recursion_counters:
            self.recursion_counters[agent_name] = 0

        self.recursion_counters[agent_name] += 1

        if self.recursion_counters[agent_name] > 2:
            logger.error(f"❌ Limite de recursão atingido para agente {agent_name}")
            return {
        ...
        try:
            ...
            salvar_erro(f"agente_{agent_name}", e, contexto={'data': data})
            return {'error': str(e), 'agent': agent_name}
        finally:
            # CORREÇÃO: Reset contador de recursão após execução (bem-sucedida ou com erro)
            if agent_name in self.recursion_counters:
                self.recursion_counters[agent_name] = 0
    ...
    def _consolidate_psychological_analysis(self, agents_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'visceral_master' in agents_results:
            ...
            if isinstance(visceral_data, dict) and 'avatar_visceral' in visceral_data:
        ...
        if 'anti_objection' in agents_results:
            ...
            # Correção: verificar se é dict e se a chave existe
            if isinstance(anti_obj_data, dict) and 'sistema_anti_objecao' in anti_obj_data:
                consolidated['sistema_anti_objecao_completo'] = anti_obj_data['sistema_anti_objecao']
            elif isinstance(anti_obj_data, dict): # Caso o retorno seja diretamente o dict desejado
        ...
        if 'visual_director' in agents_results:
            ...
            if isinstance(visual_data, dict) and 'provas_visuais' in visual_data:
        ...
        if 'pre_pitch_architect' in agents_results:
            ...
            if isinstance(pre_pitch_data, dict) and 'pre_pitch_invisivel' in pre_pitch_data:
                consolidated['pre_pitch_orquestrado'] = pre_pitch_data['pre_pitch_invisivel']
            elif isinstance(pre_pitch_data, dict): # Caso o retorno seja diretamente o dict desejado
    ...
    def _calculate_psychological_metrics(self, agents_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            'agentes_executados': len([r for r in agents_results.values() if not isinstance(r, dict) or r.get('status') != 'failed']),
        ...
        for agent_result in agents_results.values():
            if isinstance(agent_result, dict):
                if 'drivers_customizados' in agent_result and isinstance(agent_result['drivers_customizados'], list):
                    total_drivers += len(agent_result['drivers_customizados'])
                if 'provas_visuais' in agent_result and isinstance(agent_result['provas_visuais'], list):
                    total_provas += len(agent_result['provas_visuais'])
                if 'scripts_personalizados' in agent_result and isinstance(agent_result['scripts_personalizados'], list):
                    total_scripts += len(agent_result['scripts_personalizados'])
                # Incluir métricas de anti-objection e pre_pitch se disponíveis
                if 'sistema_anti_objecao' in agent_result and isinstance(agent_result['sistema_anti_objecao'], dict):
                    if 'objecoes_universais' in agent_result['sistema_anti_objecao']:
                        total_scripts += len(agent_result['sistema_anti_objecao']['objecoes_universais'].get('tempo', {}).get('scripts_personalizados', []))
                        total_scripts += len(agent_result['sistema_anti_objecao']['objecoes_universais'].get('dinheiro', {}).get('scripts_personalizados', []))
                        total_scripts += len(agent_result['sistema_anti_objecao']['objecoes_universais'].get('confianca', {}).get('scripts_personalizados', []))
                if 'pre_pitch_invisivel' in agent_result and isinstance(agent_result['pre_pitch_invisivel'], dict):
                    if 'roteiro_completo' in agent_result['pre_pitch_invisivel'] and 'abertura' in agent_result['pre_pitch_invisivel']['roteiro_completo']:
                         if 'script' in agent_result['pre_pitch_invisivel']['roteiro_completo']['abertura']:
                             total_scripts += 1 # Contabiliza o script de abertura
...
"""
    ...
    def _process_archaeological_response(self, response: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "```json" in response:
                ...
                # CORREÇÃO: Utiliza a chave correta para o array de insights
                parsed_response = json.loads(json_text)
                if 'insights_escavados' not in parsed_response:
                    parsed_response['insights_escavados'] = []
                if 'metricas_forenses' not in parsed_response:
                    parsed_response['metricas_forenses'] = {}
                return parsed_response
            else:
                # Análise estruturada do texto
                return self._extract_archaeological_insights(response, data)
        except json.JSONDecodeError as e:
            logger.error(f"Erro ao decodificar JSON na resposta do Arqueólogo: {e}")
            return self._generate_archaeological_fallback(data)
        except Exception as e:
            logger.error(f"Erro inesperado ao processar resposta do Arqueólogo: {e}")
            return self._generate_archaeological_fallback(data)
    ...
    def _extract_insights_from_text(self, text: str) -> List[str]:
        ...
        # CORREÇÃO: Tratamento para evitar erro com lista vazia ou índice inválido
    ...
    def _extract_metrics_from_text(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'intensidade_linguistica': len([w for w in text.split() if w.isupper()]) / len(text.split()) * 100 if text.split() else 0
        }
    ...
    def _process_visceral_response(self, response: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if "```json" in response:
                ...
                json_text = response[start:end].strip()
                # CORREÇÃO: Trata erro de decodificação JSON
                return json.loads(json_text)
            else:
                return self._extract_visceral_insights(response, data)
        except json.JSONDecodeError as e:
            logger.error(f"Erro ao decodificar JSON na resposta do Mestre Visceral: {e}")
            return self._generate_visceral_fallback(data)
        except Exception as e:
            logger.error(f"Erro inesperado ao processar resposta do Mestre Visceral: {e}")
            return self._generate_visceral_fallback(data)
...
"""
    ...
    def _process_drivers_response(self, response: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao decodificar JSON na resposta do Arquiteto de Drivers: {e}")
            return self._generate_drivers_fallback(data)
        except Exception as e:
            logger.error(f"Erro inesperado ao processar resposta do Arquiteto de Drivers: {e}")
            return self._generate_drivers_fallback(data)
...
"""
    ...
    def _process_visual_response(self, response: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao decodificar JSON na resposta do Diretor Visual: {e}")
            return self._generate_visual_fallback(data)
        except Exception as e:
            logger.error(f"Erro inesperado ao processar resposta do Diretor Visual: {e}")
            return self._generate_visual_fallback(data)
...
"""
    ...
    def _process_anti_objection_response(self, response: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao decodificar JSON na resposta do Especialista Anti-Objeção: {e}")
            return self._generate_anti_objection_fallback(data)
        except Exception as e:
            logger.error(f"Erro inesperado ao processar resposta do Especialista Anti-Objeção: {e}")
            return self._generate_anti_objection_fallback(data)
...
"""
    ...
    def _process_pre_pitch_response(self, response: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao decodificar JSON na resposta do Mestre de Pré-Pitch: {e}")
            return self._generate_pre_pitch_fallback(data)
        except Exception as e:
            logger.error(f"Erro inesperado ao processar resposta do Mestre de Pré-Pitch: {e}")
            return self._generate_pre_pitch_fallback(data)
